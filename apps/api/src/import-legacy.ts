...
async function runIfTablesExist(names: string[], label: string, sql: string) {
...
async function main() {
  ...
  await runIfTablesExist(
    ...
    "category images",
    `INSERT INTO category_images (id, category_id, url, alt, is_primary, sort_order, created_at)
