...
async function ensureAdmin() {
  ...
  const existing = await query<Row>("SELECT id FROM users WHERE email = ? LIMIT 1", [email]);
  ...
  await execute(
    ...
    [id("usr"), "Sunspark Admin", email, passwordHash]
  );
}

async function ensureCategory(name: string, description: string, sortOrder: number) {
  const slug = slugify(name);
  const existing = await query<Row>("SELECT id FROM categories WHERE slug = ? LIMIT 1", [slug]);
  ...
  await execute(
    ...
    [id("cat"), name, slug, description, sortOrder]
  );
}
