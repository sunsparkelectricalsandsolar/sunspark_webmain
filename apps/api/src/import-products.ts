...
type ProductOptionImport = {
  productKey: string;
  optionLabel: string;
  sellingUnit: string;
  buyingPriceKsh: number;
  sellingPriceKsh: number;
  comparePriceKsh: number | null;
  isDefault: boolean;
};
...
function readExport(path: string): TableExport {
  const fullPath = resolve(path);
  if (!existsSync(fullPath)) throw new Error(`Import file not found: ${fullPath}`);

  const parsed = JSON.parse(readFileSync(fullPath, "utf8")) as TableExport;
...
function rowsAsObjects<T>(table: TableExport, mapper: (row: Record<string, unknown>) => T) {
...
function normalizeUnit(value: string) {
  ...
  if (["METRE", "METER", "PER METRE", "PER METER"].includes(normalized)) return "METRE";
...
function optionId(productId: string, label: string, unit: string) {
  return `opt_${slugify(`${productId}-${label}-${unit}`)}`.slice(0, 191);
}

async function existingId(connection: Awaited<ReturnType<typeof pool.getConnection>>, table: string, column: string, value: string) {
  const rows = await connection.query(`SELECT id FROM ${table} WHERE ${column} = ? LIMIT 1`, [value]) as { id: string }[];
  return rows[0]?.id ?? null;
}

async function ensureCategory(connection: Awaited<ReturnType<typeof pool.getConnection>>, name: string) {
  ...
  await connection.query(
    "INSERT INTO categories (id, name, slug, description, is_active, sort_order) VALUES (?, ?, ?, ?, TRUE, ?)",
    [categoryId, name, slug, `${name} products available from Sunspark Electricals and Solar.`, 100]
  );
  return categoryId;
}

function mapProduct(row: Record<string, unknown>): ProductImport {
...
function mapOption(row: Record<string, unknown>): ProductOptionImport {
  return {
...
async function main() {
  ...
  const optionsByProduct = options.reduce((map, option) => {
    ...
    return map;
  }, new Map<string, ProductOptionImport[]>());
